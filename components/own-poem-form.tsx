  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <Card className="border-primary/15 shadow-sm"><CardHeader className="border-b border-border/70 bg-accent/20"><CardTitle className="font-serif text-xl">Your poem</CardTitle><p className="text-sm text-muted-foreground">Share the original text and where it came from.</p></CardHeader><CardContent className="space-y-5 pt-5">
        ...
        <label className="space-y-2 text-sm font-medium"><span>Source</span><Input type="text" name="source" required autoComplete="off" value={source} onChange={(e) => setSource(e.target.value)} placeholder="Book, website, personal knowledge..." /><span className="block text-xs font-normal text-muted-foreground">This helps moderators verify it&apos;s really yours before publishing.</span></label>
